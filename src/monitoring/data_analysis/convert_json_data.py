...
class ConvertJsonData:
    ...
    def __init__(self):
        ...
        self.combined_simulation_entity_data_df = self.get_combined_simulation_df()

    def get_entity_data(self) -> pd.DataFrame:
        """Create a df with all entities and their starting stats"""
        with open(ANALYSIS_SOLUTION / "entity_data.json", 'r',
    ...
    def get_df_goods_entering_production(self) -> pd.DataFrame:
        ...
        with open(ANALYSIS_SOLUTION / "data_goods_entering_production.json", 'r', encoding='utf-8') as receipt_products:
    ...
    def get_df_finished_products_leaving_production(self) -> pd.DataFrame:
        ...
        with open(ANALYSIS_SOLUTION / "data_finished_products_leaving_production.json", 'r',
                  encoding='utf-8') as finished_products:
            data = json.load(finished_products)
            return pd.DataFrame(data)

    def get_combined_simulation_df(folder_path: Union[str, Path]) -> pd.DataFrame:

        folder = Path(ENTITIES_DURING_SIMULATION_DATA)
